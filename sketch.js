...
function renderWaves(omega_,x_spacing_,wave_width_){
    ...
    for( var i=0; i<=N; i++ ) {
      noFill()
      curveVertex(i*x_spacing_+600,x_axis_1-(22.5*.613037)*cos(i*d_theta))
      pop()
    }
    endShape()
    ...
    for( var i=0; i<=N; i++ ) {
      noFill()
      curveVertex(i*x_spacing_+600,x_axis_2-(22.5*.792113)*cos(i*d_theta))
      pop()
    }
    endShape()
    ...
    for( var i=0; i<=N; i++ ) {
      noFill()
      curveVertex(i*x_spacing_+600,x_axis_3-(22.5)*cos(i*d_theta))
      pop()
    }
    endShape()
}
